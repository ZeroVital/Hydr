...
bool FHydrGameHelpers::DeprojectScreenToWorld(const FVector2D& ScreenPosition, ULocalPlayer* Player, FVector& RayOrigin, FVector& RayDirection)
{
	if (Player != NULL && Player->ViewportClient != NULL && Player->ViewportClient->Viewport != NULL && Player->PlayerController != NULL)
	{
		...
		if (Player->GetProjectionData(Player->ViewportClient->Viewport, eSSP_FULL,  ProjectionData))
		{
			...
			FSceneView::DeprojectScreenToWorld(ScreenPosition, ProjectionData.GetConstrainedViewRect(), InvViewMatrix, InvProjectionMatrix, RayOrigin,  RayDirection);
...
FVector FHydrGameHelpers::IntersectRayWithPlane(const FVector& RayOrigin, const FVector& RayDirection, const FPlane& Plane)
{
...
TSharedPtr<TArray<uint8>> FHydrGameHelpers::CreateAlphaMapFromTexture(UTexture2D* Texture)
{
...
FCanvasUVTri FHydrGameHelpers::CreateCanvasTri(FVector2D V0, FVector2D V1, FVector2D V2)
{
	FCanvasUVTri OutTri;
	OutTri.V0_Pos = V0;
	OutTri.V1_Pos = V1;
