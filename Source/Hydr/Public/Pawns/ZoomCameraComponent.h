...
class UZoomCameraComponent : public UCameraComponent
{
	GENERATED_UCLASS_BODY()

public:

	// Begin UZoomCameraComponent interface

	virtual void GetCameraView(float DeltaTime, FMinimalViewInfo& DesiredView) override;

	// End UZoomCameraComponent interface
	...
	*/
	void UpdateCameraMovement(const APlayerController1* InPlayerController);
	...
	*/
	void ClampCameraLocation(const APlayerController1* InPlayerController, FVector& OutCameraLocation);
	...
	* @param	DownTime			Time pressed
	*/
	void OnPinchUpdate(class UHydrInput* InputHandler, const FVector2D& ScreenPosition1, const FVector2D& ScreenPosition2, float DownTime);
...
private:
	...
	APlayerController1* GetPlayerController();

	/* Update the movement bounds of this component. */
	void UpdateCameraBounds(const APlayerController1* InPlayerController);
